...
export async function POST(req:Request) {
  try {
    ...
    const memberResponse = await supabaseFetch(`choir_members?select=id,full_name&active=eq.true&full_name=ilike.${encodeURIComponent(cleanName)}&pin=eq.${encodeURIComponent(pin)}&limit=1`)
